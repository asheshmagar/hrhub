		...
		<DayPicker
			...
			components={{
				...
				Dropdown: ({ ...props }) => {
					...
					if (!['months', 'years'].includes(props.name ?? '')) return null;
					let selectItems: Array<{ label: string; value: string }> = [];
					if ('months' === props.name) {
						...
						if (earlierYear && latestYear) {
							const yearsLength = latestYear - earlierYear + 1;
							selectItems = Array.from({ length: yearsLength }, (_, i) => ({
								value: (earlierYear + i).toString(),
								label: (earlierYear + i).toString(),
							}));
						}
					}
					...
					return (
						<Select
							onValueChange={(v) => {
								...
								if (props.name === 'years') {
									nextDate.setFullYear(parseInt(v));
								} else {
									nextDate.setMonth(parseInt(v));
								}
								goToMonth(nextDate);
							}}
